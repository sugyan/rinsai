...
> ⚠️ **Status: very early.** rinsai speaks USI and searches: iterative-deepening negamax αβ over a material evaluation, with quiescence, a transposition table and 千日手/連続王手 detection. The repository also carries its own measurement loop — frozen floodgate-derived opening sets, a match harness refereeing on an independent rules library — fixed nodes or a real clock — and a pentanomial SPRT driver, which has run its first real test and passed it. Interior nodes and quiescence order their captures by what they win, an interior node tries the quiet moves that refuted a sibling before the rest and ranks the others by how they have done elsewhere, late quiet moves are searched a ply shallower, a check that does not answer one is searched a ply deeper, and every move behind the first is scouted on a one-point window; it plays legal games, which is not a strength claim. The plan is in [DESIGN.md](./DESIGN.md) (route, repository structure, the staged roadmap E0–E6), what is frozen in [CONVENTIONS.md](./CONVENTIONS.md), and why it is this way and not that way — with the measurements each choice rests on — in [FAQ.md](./FAQ.md). What is open and what is next are the issues.
