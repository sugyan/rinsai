...
5b. **PVS at interior nodes** — every move behind the first is scouted on a one-point window, and widens back to the node's own only by proving it deserves to. A number of its own rather than a place in the order, because it arrived after item 5 and renumbering the list would orphan every reference to an item by number. ⚠️ **The root keeps its own window**: item 9 owes `negamax_root` the return type that tells a fail-low from an abandoned iteration, and scouting root moves before that exists is the bug that item describes.
