...
6. Check extension — **more important than in chess**; watch its interference with perpetual-check repetition, which is a search-explosion risk. ⚠️ **A node in check extends nothing, and that is the bound**: every extended ply is followed by one that is not, so a line still spends a ply of depth every two plies. **The root does not extend**; extending there is its own SPRT.
